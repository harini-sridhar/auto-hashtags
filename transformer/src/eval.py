...
def generate(args):
    ...
    opt.infos_path = args['infos_path']
    opt.image_folder = args['image_folder']
    opt.model = args['model']
    opt.num_images = args['num_images']
    opt.num_tags = args['num_tags']
    ...
    glove_input_file = 'src/models/glove/glove.6B/glove.6B.100d.txt'
    word2vec_output_file = 'src/models/glove/glove.6B/glove.6B.100d.txt.word2vec'
    glove2word2vec(glove_input_file, word2vec_output_file)

    # Load the popular instagram hashtag file and extract the categories
    with open('src/models/glove/hashtag.pkl', 'rb') as f:
        hashtag = cPickle.load(f)

    # Load the banned hashtags
    with open('src/models/glove/banned.pkl', 'rb') as f:
    ...
    with open('src/models/glove/generic_hashtags.pkl', 'rb') as f:
    ...
    hashstr = ' '.join(tag for tag in hash_instatags)
    return hashstr
