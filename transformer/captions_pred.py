...
def remove_stopwords(caption):
    ...
    for p in string.punctuation:
        stopWords.add(p)
    # Add start/end tokens to stopWords
    #stopWords.add('<start>')
    #stopWords.add('<end>')
...
def similarity_scores(caption, hashtag):
    ...
    # Use the glove model
    model = KeyedVectors.load_word2vec_format('./models/glove/glove.6B/glove.6B.100d.txt.word2vec', binary=False, limit=100000)
    ...
    for token in caption:
        ...
        #print ('model sim:', token, best_category, model.similarity(token, best_category))
...
def get_hashtags (caption, hashtag): #need to add number of hashtags
    best_category_matches = similarity_scores(caption, hashtag)
    #st.write (best_category_matches)
    insta_tags=[]
